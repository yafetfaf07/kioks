...
export class ProductController {
  ...
  updateProduct:RequestHandler<{id:string}, unknown, updateProduct, unknown>=async(req, res,next) => {
    try {
      const id = new mongoose.Types.ObjectId(req.params.id);

      const update = req.body;

      const updateProduct = await this._services.updateProduct(id, update);
  ...
  getProductByNameAndMerchantId: RequestHandler<findProductByName, unknown,unknown> = async(req,res,next) => {
    try {
      const id = new mongoose.Types.ObjectId(req.params.mid);
      const name = req.params.name;
      const foundProduct = await this._services.getProductByMerchantName(name,id);
